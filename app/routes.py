...
def Lesson1QuestionCheck():
    ...
    for u, l, q in db.session.query(Users, Lesson, Question).filter(Users.id==Lesson.user_id).filter(Lesson.id==Question.lesson_id).filter(Users.id==current_user.id).filter(Question.QuestionNumber==1).all():
        if form.Question1.data == q.Answer:
            q.Answered = True
            db.session.commit()
    return redirect(url_for('Lesson1'))

@app.route('/Lesson2QuestionCheck', methods=['GET'])
def Lesson2QuestionCheck():
    form = UserQuestionCheck()
...
@app.route('/Lesson3QuestionCheck', methods=['GET'])
def Lesson3QuestionCheck():
    form = UserQuestionCheck()
...
@app.route('/Lockpick_Intro')
def Lockpick_Intro():
    return render_template("Lockpick_Intro.html")